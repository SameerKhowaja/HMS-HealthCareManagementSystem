...
class DoctorController extends Controller
{
    ...
    function patientTreatmentSave($id,Request $req){
        ...
        if($patient->count() && $doctor->count()){
            ...
            $treatment->medical_condition = strtolower($req->medical_condition);
    ...
    function performResearch(Request $req){
        ...
        if($req->var2 == 'age'){
            $data = [   'type'=>'age',
                        'disease'=>strtolower($req->disease),
            ...
            $research_data = Treatment::where("medical_condition",'like',"%".strtolower($req->disease)."%")
            ->with(["patient","patient.hospital_data"])
            ->get();
